...
Source: <https://codelabs.developers.google.com/secure-tdd-vibecoding-antigravity>
...
Rather than treating security as a late-stage gate, you will learn to "shift security left" to the point of code inception. You will enforce development standards, automate [STRIDE threat modeling](https://owasp.org/www-community/Threat_Modeling_Process#stride-threat--mitigation-techniques), and gate agent actions with git pre-commit hooks and agent-specific execution hooks.
...
- Configure Git pre-commit hooks to automate [Semgrep](https://semgrep.dev/) scans with local remediation loops in Antigravity.

## What you'll need

- A web browser such as [Chrome](https://www.google.com/chrome/)
- Familiarity with Python, Pytest, and basic terminal commands
- **Google Antigravity IDE** installed. See the [official website](https://antigravity.google/docs/home).
- **uv** package manager installed. See [uv installation guide](https://docs.astral.sh/uv/getting-started/installation/).
- **Git** command line tool installed. Git is used exclusively for local version control in this lab, so a GitHub account is not required. See the [Git installation guide](https://github.com/git-guides/install-git).
...
**Local Concept Demonstration:** Because this lab focuses entirely on demonstrating local development guardrails and pre-commit remediation loops, everything runs locally on your workstation. There is no need to authenticate to Google Cloud or connect your repository to GitHub!

Provide your authentication credentials for the agent to call Gemini models. Obtain a standard Gemini API key from [Google AI Studio](https://aistudio.google.com/app/apikey) and export it in your IDE terminal session:
...
**Note:** Because Antigravity generates implementations dynamically, the code in your `app/agent.py` file may be structured slightly differently than the snippet below, but it will share the same core ADK 2.0 concepts.
...
**Caution:** Failing to centralize these rules causes the agent to fall back on basic training data, which might result in vulnerable string concatenation or open redirect vectors.
...
**Tip:** The `--error` flag is **critical**; without it, Semgrep will log the security finding to standard output but return exit code `0` (success by default), which allows the unsafe commit to pass. Pre-commit only blocks commits on non-zero exit codes.
...
**Caution:** To safely intercept tool executions like `run_command`, the `PreToolUse` event definition MUST include the mandatory `matcher` field. Leaving this out or misconfiguring it can let the agent execute raw, unchecked shell actions.
...
**Commit Hygiene:** Do not use the `--no-verify` flag when executing the commit command! We specifically want the pre-commit hook to fire so you can observe Semgrep intercept the simulated API key leak.
...
3.  Open the provided URL in your web browser to start chatting with your Shopping Assistant agent interactively. Try asking it to redeem a discount code:
    ...
    **Note:** Because Antigravity generates tool logic dynamically, your agent's implementation of the registered user check may vary. If the agent rejects `user_123` as unregistered (e.g., `User ID 'user_123' is not registered`), ask the agent which user IDs are registered in its store (or check `app/agent.py`), and retry with a valid registered user ID!
...
- Explore [Antigravity Customizing Guide](https://cloud.google.com/vertex-ai) for advanced configurations.
- Read more about [STRIDE Threat Modeling](https://en.wikipedia.org/wiki/STRIDE_(security)).
- Implement Semgrep rules tailored to your organization's security policies.
